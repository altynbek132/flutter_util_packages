  ...
  group('Reaction', () {
    ...
    test('works with delay', () {
      ...
      fakeAsync((async) {
        ...
        async.elapse(const Duration(milliseconds: 500)); // should now trigger effect
        expect(executed, isTrue);
    ...
    test('that fires immediately with delay', () {
      ...
      fakeAsync((async) {
        ...
        expect(executed, isFalse);
        async.elapse(const Duration(milliseconds: 1000)); // should now trigger effect
        expect(executed, isTrue);
    ...
    test('fires onError on exception inside effect', () {
      ...
      expect(thrown, isTrue);
      expect(dispose.reaction.errorValue, isException);
      expect(dispose.reaction.errorValue.toString(), contains('MobXCaughtException'));
      dispose();
    });
    ...
    test('reaction throws when config.disableErrorBoundaries = true', () {
      final context = ReactiveContext(config: ReactiveConfig(disableErrorBoundaries: true));

      expect(() {
        final dispose = reaction((_) => throw Exception('FAIL'), (_) {}, context: context);
    ...
    group('start/endTracking', () {
      ...
      test('ReactionImpl tracks observables', () {
        final reaction = ReactionImpl(mainContext, () {}, name: 'test_reaction_1')..track(() {});

        expect(reaction.hasObservables, isFalse);

        final x = Observable(0);
        final reaction1 = ReactionImpl(mainContext, () {}, name: 'test_reaction_2')..track(() => x.value + 1);
      ...
      test('when disposed, clearObservables', () {
        ...
        final reaction = ReactionImpl(mainContext, () {}, name: 'test_reaction_1')..track(() => x.value + 1);
