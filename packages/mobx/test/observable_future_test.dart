...
void main() {
  ...
  group('ObservableFuture', () {
    ...
    test('status should be pending while running and fulfilled when completed', () async {
      final completer = Completer<String>();
      final future = ObservableFuture(completer.future);
    ...
    test('status should be pending while running and rejected when failed', () async {
    ...
    test('replace should update value and status after new promise completes', () async {
    ...
    test('match return null if state is pending and no pending matcher is provided', () async {
      final future = ObservableFuture<int>(Future(() => 0));
    ...
    test('match return null if state is fulfilled and no fulfilled matcher is provided', () async {
      final future = ObservableFuture<int>(Future(() => 0));
    ...
    test('match return null if state is rejected and no rejected matcher is provided', () async {
      final completer = Completer<int>();
      final future = ObservableFuture<int>(completer.future);
    ...
    test('timeout works', () async {
      final completer = Completer<int>();
      final future = ObservableFuture(completer.future).timeout(const Duration(milliseconds: 1));

      expect(() async => await future, throwsA(const TypeMatcher<TimeoutException>()));
    });
