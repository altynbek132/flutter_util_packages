...
void main() {
  ...
  group('ObservableStream', () {
    ...
    test('omitting done in match uses value instead for values', () async {
      ...
      autorun((_) {
        values.add(stream.match(waiting: () => 'waiting', active: (i) => 'value/done')!);
      });
    ...
    test('pauses and unpauses when starting/stopping observation', () async {
      ...
      final future = asyncWhen((_) => stream.status == StreamStatus.done || stream.value == 2);
    ...
    test('listen() receives all events if source is non-broadcast', () async {
      ...
      ctrl.add(2);
      await pumpEventQueue();
      expect(subValues, equals([0, 1, 2]), reason: 'non-broadcast streams buffer events');
      ...
      ctrl.add(4);
      await pumpEventQueue();
      expect(subValues, equals([0, 1, 2, 3]), reason: 'subscription itself is paused, does not receive events');

      subscription.resume();

      ctrl.add(5);
      await pumpEventQueue();
      expect(subValues, equals([0, 1, 2, 3, 4, 5]), reason: 'receives all events emitted during pause');

      await subscription.cancel();

      ctrl.add(6);
      await pumpEventQueue();
      expect(subValues, equals([0, 1, 2, 3, 4, 5]), reason: 'sub cancelled, no longer receives events');
    });
    ...
    test('listen() and observation both keep values updated', () async {
      ...
      expect(subValues, equals([0, 1, 2, 3]), reason: 'non-broadcast streams buffer events');
    ...
    test('listen() also keeps values updated for broadcast streams', () async {
      ...
      ctrl.add(1);
      await pumpEventQueue();
      expect(ctrl.isPaused, isFalse, reason: 'broadcast ctrl itself does not pause');
      expect(stream.value, equals(0));
      ...
      await pumpEventQueue();
      expect(stream.value, equals(2));
      expect(sub1Values, equals([2]), reason: 'broadcast streams do not buffer events');
    ...
    test('implicit subscriptions keep observable values updated', () async {
      ...
      await pumpEventQueue();
      expect(ctrl.hasListener, isFalse);
      expect(ctrl.isPaused, isFalse, reason: 'no more listeners, no longer paused');
      expect(stream.value, equals(4));
    ...
    test('cancelOnError cancels the stream on first error', () async {
      final ctrl = StreamController<int>();
      final stream = ObservableStream(ctrl.stream, initialValue: 0, cancelOnError: true);
    ...
    test('cannot observe non-broadcast stream once sub is cancelled', () async {
      ...
      final future2 = asyncWhen((_) => stream.value == 3);
      expect(future2, throwsA(isA<MobXCaughtException>()), reason: 'sub on non-broadcast stream already cancelled');
    });

    test('can observe value with custom equals', () async {
      final ctrl = StreamController<int>();
      final stream = ObservableStream(ctrl.stream, initialValue: 3, equals: (_, __) => false);
    ...
    <String, StreamTestBody>{
      ...
      'asyncExpand': (s) => s.asyncExpand((n) => Stream.fromIterable(Iterable<int>.generate(n))),
      ...
      'timeout': (s) => s.timeout(const Duration(seconds: 0), onTimeout: (n) => true),
      'transform': (s) => s.transform(StreamTransformer.fromHandlers(handleData: (data, sink) {
            sink.add(data);
          })),
    ...
    <String, Case>{
      ...
      'pipe': futureCase((s) => s.pipe(StreamController.broadcast()), null as dynamic),
...
Case<F, R> futureCase<R, F extends ObservableFuture<R> Function(ObservableStream<int>)>(F body, R result,
        {int length = 10}) =>
...
void testStreamCombinator<T>(String description, StreamTestBody body) {
  test(description, () async {
    final stream = ObservableStream(Stream.fromIterable(Iterable<int>.generate(10)));
...
void testStreamToFutureCombinator<T>(String description, Case testCase) {
  test(description, () async {
    final stream = ObservableStream(Stream.fromIterable(Iterable<int>.generate(testCase.length)));
