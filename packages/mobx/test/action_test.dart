...
void main() {
  ...
  group('Action', () {
    ...
    test('modifications are picked up', () {
      ...
      var total = 0;
      final a = Action(() {
        x.value = x.value + 1; // No reaction-infinite-loop as we are not tracking the observables
      });
    ...
    test('uses provided context', () {
      ...
      mock.when(() => context.startAllowStateChanges(allow: mock.any(named: 'allow'))).thenReturn(true);
    ...
    test('on mainContext, should throw if mutating outside an action, with observers', () {
      mainContext.config = ReactiveConfig(writePolicy: ReactiveWritePolicy.observed);
    ...
    test('on custom context, should throw if mutating outside an action, with observers', () {
      final context = ReactiveContext(
          config: ReactiveConfig(disableErrorBoundaries: true, writePolicy: ReactiveWritePolicy.observed));
    ...
    test('should throw if mutating outside an action, when always enforced', () {
      final context = ReactiveContext(
          config: ReactiveConfig(disableErrorBoundaries: true, writePolicy: ReactiveWritePolicy.always));
      final x = Observable(0, context: context);
