...
void main() {
  ...
  group('ActionController', () {
    ...
    test('startAction calls startUntracked, startBatch and startAllowStateChanges', () {
      final context = MockContext();
      when(() => context.nameFor(any())).thenReturn('Test-Action');
      when(() => context.startAllowStateChanges(allow: any(named: 'allow'))).thenReturn(true);
    ...
    test('endAction calls endAllowStateChanges, endBatch and endUntracked', () {
      ...
      final runInfo = ActionRunInfo(
          name: 'test', prevDerivation: prevDerivation, prevAllowStateChanges: false, startTime: DateTime.now());
