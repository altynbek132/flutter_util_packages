...
void main() {
  ...
  group('ReactiveContext', () {
    ...
    test('throws Exception for reactions that do not converge', () {
      ...
      expect(() => runInAction(() => a.value = 1), throwsA(const TypeMatcher<MobXCyclicReactionException>()));
      d();
    });
    ...
    group('conditionallyRunInAction', () {
      ...
      test('when running OUTSIDE an Action, it should USE the ActionController', () {
        ...
        mock.when(() => controller.startAction(name: mock.any(named: 'name'))).thenReturn(MockActionRunInfo());
        ...
        mock.verifyInOrder([() => controller.startAction(), () => controller.endAction(mock.any())]);
        expect(hasRun, isTrue);
      });

      test('when no ActionController is provided, it should create an ad-hoc ActionController', () {
        final context = createContext();
        var hasRun = false;
        final o = Observable(0);
      ...
      test('when running INSIDE an Action, it should NOT USE the ActionController', () {
        final controller = MockActionController();
        final context = createContext();
        final o = Observable(0);
  ...
  group('ReactiveConfig', () {
    test('clone works', () {
      ...
      expect(clone.disableErrorBoundaries == config.disableErrorBoundaries, isTrue);
    ...
    test('when no overrides are provided the clone reuses source values', () {
      ...
      expect(clone.disableErrorBoundaries, equals(config.disableErrorBoundaries));
