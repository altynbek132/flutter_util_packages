...
void main() {
  ...
  group('ObservableMap', () {
    ...
    test('splayTreeMapFrom iterates based on compare function', () {
      final map = ObservableMap<String, int>.splayTreeMapFrom({}, compare: (a, b) => b.compareTo(a));
      map['a'] = 1;
      map['b'] = 2;
      map['c'] = 3;
    ...
    group('fires reportObserved() for iterable transformation methods only when iterating', () {
    ...
    group('fires reportChanged() for write methods', () {
      <String, void Function(ObservableMap<String, int>)>{
        ...
        'addEntries': (m) => m.addEntries([const MapEntry('d', 4), const MapEntry('e', 5)]),
        'clear': (m) => m.clear(),
        'remove': (m) => m.remove('a'),
        'removeWhere': (m) => m.removeWhere((key, i) => i == 2),
        'update': (m) => m.update('a', (value) => value + 1),
        'update ifAbsent': (m) => m.update('d', (value) => value + 1, ifAbsent: () => 4),
    ...
    test('remove reports remove change when there are listeners and the item exists', () {
      late MapChange change;
    ...
    test("remove doesn't report changes when there are listeners and the item doesn't exist", () {
      MapChange? change;
      ObservableMap.of({'a': 0})
        ..observe((c) => change = c)
        ..remove('b');
    ...
    test('observe sends changes immediately when fireImmediately is true', () {
      final changes = <MapChange>[];
      ObservableMap.of({'a': 0, 'b': 1}).observe(changes.add, fireImmediately: true);
    ...
    test('bypass observable system', () {
      ...
      expect(nonObservableInnerLength, equals(0), reason: 'should not be observable');
    });
  });
}

void runWriteTest(String description, void Function(ObservableMap<String, int>) body) {
  test(description, () {
    final atom = MockAtom();
...
void runReadTest(String description, void Function(ObservableMap<String, int>) body) {
  test(description, () {
    final atom = MockAtom();
    final map = wrapInObservableMap(atom, {'a': 1, 'b': 2, 'c': 3});
...
void runIterableTest(String description, Iterable Function(ObservableMap<String, int>) body) {
  test(description, () {
    final atom = MockAtom();
    final map = wrapInObservableMap(atom, {'a': 1, 'b': 2, 'c': 3});
