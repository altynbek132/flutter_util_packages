...
class ObservableList<T>
  ...
  ObservableList.of(Iterable<T> elements, {ReactiveContext? context, String? name})
      : this._wrap(context, _observableListAtom<T>(context, name), List<T>.of(elements, growable: true));

  ObservableList._wrap(ReactiveContext? context, this._atom, this._list) : _context = context ?? mainContext;
  ...
  Listeners<ListChange<T>> get _listeners => _listenersField ??= Listeners(_context);
  ...
  Dispose observe(Listener<ListChange<T>> listener, {bool fireImmediately = false}) {
    if (fireImmediately == true) {
      final change = ListChange<T>(
          list: this, rangeChanges: <RangeChange<T>>[RangeChange(index: 0, newValues: toList(growable: false))]);
      listener(change);
    }
  ...
  void _notifyElementUpdate(int index, T? newValue, T? oldValue, {OperationType type = OperationType.update}) {
    ...
    final change = ListChange<T>(list: this, elementChanges: <ElementChange<T>>[
      ElementChange(index: index, newValue: newValue, oldValue: oldValue, type: type)
    ]);
  ...
  void _notifyRangeUpdate(int index, List<T>? newValues, List<T>? oldValues) {
    ...
    final change = ListChange<T>(
        list: this,
        rangeChanges: <RangeChange<T>>[RangeChange(index: index, newValues: newValues, oldValues: oldValues)]);

    _listeners.notifyListeners(change);
  }
}

typedef ListChangeListener<TNotification> = void Function(ListChange<TNotification>);
...
class ElementChange<T> {
  ElementChange({required this.index, this.type = OperationType.update, this.newValue, this.oldValue});
...
@visibleForTesting
ObservableList<T> wrapInObservableList<T>(Atom atom, List<T> list) => ObservableList._wrap(mainContext, atom, list);
